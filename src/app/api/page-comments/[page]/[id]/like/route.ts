...
// Like toggle — bitta sessiya bir izohga bir marta.
export async function POST(
  ...
  const rl = await rateLimit(`like:page:${sid}:${ip}`, 20, 60_000);
  if (!rl.ok) return apiError(429, "rate_limit", "Iltimos, biroz kutib turing.");

  const comment = await prisma.comment.findUnique({
    where: { id },
    select: { id: true, status: true, page: true },
  });
  if (!comment || comment.status !== "APPROVED" || comment.page !== page) {
    return apiError(404, "not_found", "Izoh topilmadi");
  }
  ...
  if (existing) {
    ...
    emit(`comments:page:${page}`, { action: "unlike", id });
  ...
  emit(`comments:page:${page}`, { action: "like", id });
